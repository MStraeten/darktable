...
typedef struct dt_iop_local_contrast_rgb_gui_data_t
{
  ...
  gboolean detail_locked;
  gboolean global_locked;
  ...
  GtkWidget *detail_lock;
  GtkWidget *global_lock;
} dt_iop_local_contrast_rgb_gui_data_t;


const char *name()
{
  return _("local contrast rgb c");
}
...
static void gui_cache_init(dt_iop_module_t *self)
{
  ...
  g->detail_locked = FALSE;
  g->global_locked = FALSE;
  g->luminance_valid = FALSE;
...
static void detail_lock_callback(GtkWidget *togglebutton,
                                  ...
                                  dt_iop_module_t *self)
{
  ...
  dt_iop_local_contrast_rgb_gui_data_t *g = self->gui_data;
  g->detail_locked = !g->detail_locked;
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(togglebutton), g->detail_locked);
}


static void global_lock_callback(GtkWidget *togglebutton,
                                  GdkEventButton *event,
                                  dt_iop_module_t *self)
{
  if(darktable.gui->reset) return;
  ...
  dt_iop_local_contrast_rgb_gui_data_t *g = self->gui_data;
  g->global_locked = !g->global_locked;
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(togglebutton), g->global_locked);
}


static void detail_slider_changed(GtkWidget *slider, dt_iop_module_t *self)
{
  if(darktable.gui->reset) return;
  
  dt_iop_local_contrast_rgb_gui_data_t *g = self->gui_data;
  dt_iop_local_contrast_rgb_params_t *p = self->params;
  
  if(g->detail_locked)
  {
    float value = 0.0f;
    
    if(slider == g->detail_red)
      value = p->detail_red;
    else if(slider == g->detail_green)
      value = p->detail_green;
    else if(slider == g->detail_blue)
      value = p->detail_blue;
    
    ++darktable.gui->reset;
    dt_bauhaus_slider_set(g->detail_red, value);
    dt_bauhaus_slider_set(g->detail_green, value);
    dt_bauhaus_slider_set(g->detail_blue, value);
    --darktable.gui->reset;
  }
}


static void global_slider_changed(GtkWidget *slider, dt_iop_module_t *self)
{
  if(darktable.gui->reset) return;
  ...
  if(g->global_locked)
  {
    ...
    if(slider == g->global_red)
      value = p->global_red;
    else if(slider == g->global_green)
      value = p->global_green;
    else if(slider == g->global_blue)
      value = p->global_blue;
    
    ++darktable.gui->reset;
    dt_bauhaus_slider_set(g->global_red, value);
    dt_bauhaus_slider_set(g->global_green, value);
    dt_bauhaus_slider_set(g->global_blue, value);
    --darktable.gui->reset;
  }
}
...
void gui_update(dt_iop_module_t *self)
{
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->detail_lock), g->detail_locked);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->global_lock), g->global_locked);
}
                 ...
                 void *previous)
{
  ...
  else if(w == g->detail_red || w == g->detail_green || w == g->detail_blue)
  {
    detail_slider_changed(w, self);
  }
  else if(w == g->global_red || w == g->global_green || w == g->global_blue)
  {
    global_slider_changed(w, self);
  }
}
...
void gui_init(dt_iop_module_t *self)
{
  ...
  // Detail section with integrated lock button
  g->detail_lock = dt_iop_togglebutton_new
    (self, NULL,
     N_("lock RGB channels"), NULL, G_CALLBACK(detail_lock_callback),
     FALSE, 0, 0, dtgtk_cairo_paint_lock, NULL);
  dt_gui_add_class(g->detail_lock, "dt_transparent_background");
  dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(g->detail_lock),
                               dtgtk_cairo_paint_lock, 0, NULL);
  dt_gui_add_class(g->detail_lock, "dt_bauhaus_alignment");

  GtkWidget *detail_section_hbox = dt_gui_hbox(
    dt_gui_expand(dt_ui_section_label_new(C_("section", "details"))),
    g->detail_lock);
  gtk_widget_set_margin_top(detail_section_hbox, DT_PIXEL_APPLY_DPI(10));
  dt_gui_box_add(self->widget, detail_section_hbox);

  // Detail boost sliders
  ...
  // Global contrast section with integrated lock button
  g->global_lock = dt_iop_togglebutton_new
    (self, NULL,
     N_("lock RGB channels"), NULL, G_CALLBACK(global_lock_callback),
     FALSE, 0, 0, dtgtk_cairo_paint_lock, NULL);
  dt_gui_add_class(g->global_lock, "dt_transparent_background");
  dtgtk_togglebutton_set_paint(DTGTK_TOGGLEBUTTON(g->global_lock),
                               dtgtk_cairo_paint_lock, 0, NULL);
  dt_gui_add_class(g->global_lock, "dt_bauhaus_alignment");

  GtkWidget *global_section_hbox = dt_gui_hbox(
    dt_gui_expand(dt_ui_section_label_new(C_("section", "global contrast"))),
    g->global_lock);
  gtk_widget_set_margin_top(global_section_hbox, DT_PIXEL_APPLY_DPI(10));
  dt_gui_box_add(self->widget, global_section_hbox);

  // Global contrast boost sliders
